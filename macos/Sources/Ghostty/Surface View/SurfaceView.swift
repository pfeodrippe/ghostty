    ...
    struct SurfaceSearchOverlay: View {
        ...
        var body: some View {
            GeometryReader { geo in
                ...
                .onReceive(NotificationCenter.default.publisher(for: .ghosttySearchFocus), perform: handleSearchFocusNotification)
...
#endif
                .backport.onKeyPress(.return, action: handleReturnKey)
        }
        ...
        private func handleReturnKey(_ modifiers: EventModifiers) -> BackportKeyPressResult {
            guard surfaceView.surface != nil else { return .ignored }
        ...
        private func dispatchSearchAction(_ action: String) {
            guard let surface = surfaceView.surface else { return }
            ghostty_surface_binding_action(surface, action, UInt(action.lengthOfBytes(using: .utf8)))
        }

        private func handleSearchFocusNotification(_ notification: Foundation.Notification) {
